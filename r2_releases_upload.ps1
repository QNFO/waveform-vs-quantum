<#
.SYNOPSIS
    One-click archive uploader: copies D:\Obsidian\releases → Cloudflare R2 `releases` bucket.
...
.EXAMPLE
    .\r2_releases_upload.ps1
    .\r2_releases_upload.ps1 -Source "E:\Other\releases" -Transfers 8
    .\r2_releases_upload.ps1 -DryRun
    .\r2_releases_upload.ps1 -VerifyOnly
#>

[CmdletBinding()]
param(
    [string]$Source = "D:\Obsidian\releases",
    [int]$Transfers = 4,
    [switch]$DryRun,
    [switch]$VerifyOnly,
...
# =============================================================================
# EMBEDDED R2 CREDENTIALS (created 2026-07-15 for releases bucket)
# =============================================================================
$Script:R2_ACCESS_KEY_ID     = "6d0cb152e0f397140ffdcf38890f67c0"
$Script:R2_SECRET_ACCESS_KEY = "fb8116b3652253479956564479ef6d9ef5e3bd6d8718d64ae0a0a6dfd2c0331e"
$Script:R2_ACCOUNT_ID        = "edb167b78c9fb901ea5bca3ce58ccc4b"
$Script:R2_BUCKET            = "releases"
$Script:R2_ENDPOINT          = "https://edb167b78c9fb901ea5bca3ce58ccc4b.r2.cloudflarestorage.com"
$Script:RCLONE_REMOTE_NAME   = "releases"

# =============================================================================
# CONFIG
# =============================================================================
$Script:RCLONE_EXE    = Join-Path $PSScriptRoot "rclone.exe"
$Script:RCLONE_ZIP    = Join-Path $PSScriptRoot "rclone_releases.zip"
$Script:RCLONE_URL    = "https://downloads.rclone.org/rclone-current-windows-amd64.zip"
$Script:LOG_FILE      = Join-Path $PSScriptRoot "r2_releases_upload.log"
...
╔══════════════════════════════════════════════════════════════════════╗
║    R2 Releases Uploader — D:\Obsidian\releases → R2 releases         ║
╚══════════════════════════════════════════════════════════════════════╝
...
function Initialize-RcloneConfig {
    ...
    $env:RCLONE_CONFIG = ""
    $env:RCLONE_CONFIG_RELEASES_TYPE = "s3"
    $env:RCLONE_CONFIG_RELEASES_PROVIDER = "Cloudflare"
    $env:RCLONE_CONFIG_RELEASES_ENDPOINT = $Script:R2_ENDPOINT
    $env:RCLONE_CONFIG_RELEASES_ACCESS_KEY_ID = $Script:R2_ACCESS_KEY_ID
    $env:RCLONE_CONFIG_RELEASES_SECRET_ACCESS_KEY = $Script:R2_SECRET_ACCESS_KEY
    $env:RCLONE_CONFIG_RELEASES_REGION = "auto"
    $env:RCLONE_CONFIG_RELEASES_ACL = "private"
    Write-Step "rclone remote 'releases' configured via env vars" "Green"
}
...
function Invoke-RcloneCheck {
    ...
    $args = @(
        ...
        "--size-only",
        "--one-way",
        "--missing-on-dst", "$(Join-Path $PSScriptRoot "r2_releases_missing.txt")",
        "--error", "$(Join-Path $PSScriptRoot "r2_releases_errors.txt")"
    ) + $Script:RCLONE_FLAGS
...
function Show-FinalReport {
    ...
    $missingCount = 0
    $missingFile = Join-Path $PSScriptRoot "r2_releases_missing.txt"
    if (Test-Path $missingFile) {
        $missingCount = (Get-Content $missingFile | Where-Object { $_ -match '\S' }).Count
    }

    $errorCount = 0
    $errorFile = Join-Path $PSScriptRoot "r2_releases_errors.txt"
...
# Step 1: Download rclone (shared with archive script — skips if already present)
...
# Save a clean text report
@"
======================================================================
R2 Releases Upload Report
======================================================================
...
======================================================================
"@ | Out-File -FilePath (Join-Path $PSScriptRoot "r2_releases_report.txt") -Encoding utf8
